...
#include "libavformat/pulldetect.h"
...
typedef struct URLContext {
    ...
    int64_t w_size;
    int64_t w_rtmpsize;
    int64_t sendDateTime;
    int64_t dnsTimestampInMs;
    int64_t tcpConnectTime;
    int64_t recvTcpFirstPacketTimestamp;

    int httpReconnectTimes;                                          //http重新请求连接次数
    HttpDetecter httpDetecter[MAX_HTTP_RECONNECT_TIMES];
} URLContext;
...
int ffurl_try_open(URLContext **puc, const char *filename, int flags,
               const AVIOInterruptCB *int_cb, AVDictionary **options);
